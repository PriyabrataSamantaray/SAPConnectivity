IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[prcBuildCalendar]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[prcBuildCalendar]
GO

CREATE PROCEDURE prcBuildCalendar

AS

	delete t_FiscalDays where CalDate >= '2013-01-01'
	
	insert t_FiscalDays
	select * from t_cal where CalDate >= '2013-01-01'
