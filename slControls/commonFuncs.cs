...
namespace slControls
{
    public class commonFuncs : UserControl
    {
        ...
        public void setPSEEndpoint()
        {
            string paramUrl = endpointAddress.Uri.ToString();



            if (paramUrl.Contains("localhost"))
                paramUrl = paramUrl.Replace(@"prm/services/", @"rfcwebservices/");
            else if (paramUrl.Contains("qa"))
                paramUrl = @"http://bi-qa/rfcwebservices/PSEContinuumService.svc";
            else
                paramUrl = @"http://bi/rfcwebservices/PSEContinuumService.svc";
            ...
            endpointAddress = new EndpointAddress(paramUrl);
